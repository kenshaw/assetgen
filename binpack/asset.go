...
type asset struct {
	...
	// Name is the key used in the TOC (name by which the asset is referenced).
	Name string

	// Func is the func name to use for asset retrieval.
	Func string
}
...
func (root *assetTree) Add(route []string, a asset) {
...
func (root *assetTree) writeGoMap(w io.Writer, nindent int) {
	...
	if len(root.Children) > 0 {
		...
		for _, p := range filenames {
			indent(w, nindent+1)
			fmt.Fprintf(w, `"%s": `, p)
			root.Children[p].writeGoMap(w, nindent+1)
		}
		indent(w, nindent)
	}
...
// writeTOCAsset write a TOC entary for the given asset.
func writeTOCAsset(w io.Writer, a *asset) error {
